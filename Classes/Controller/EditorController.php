...
class EditorController
{
    ...
    public function getConfigurationAction(ServerRequestInterface $request, ResponseInterface $response = null)
    {
        ...
        if (isset($queryParameters['elements'])) {
            foreach ($queryParameters['elements'] as $element) {
                ...
                $formDataFieldName = isset($this->formData['processedTca']['columns'][$fieldName]) ? $this->formData['processedTca']['columns'][$fieldName] : '';
                $this->rteConfiguration = isset($formDataFieldName['config']['richtextConfiguration']['editor']) ? $formDataFieldName['config']['richtextConfiguration']['editor'] : [];
                ...
                foreach ($this->getExtraPlugins() as $pluginName => $config) {
                    ...
                    if (isset($editorConfiguration['extraPlugins']) && $editorConfiguration['extraPlugins'] !== null && $editorConfiguration['extraPlugins'] !== '') {
                        $editorConfiguration['extraPlugins'] .= ',';
                    }

                    if (isset($editorConfiguration['extraPlugins'])) {
    ...
    protected function prepareConfigurationForEditor(): array
    {
        ...
        if (isset($this->rteConfiguration['config'])) {
